...
def interpolate_zarr_at_timestamps(zarr_path):
    ...
    for i, (timestamp_str, pred_num, level) in enumerate(timestamp_pred_level_tuples):
        timestamp = pd.to_datetime(timestamp_str)
        print(f"Processing {i+1}/{len(timestamp_pred_level_tuples)}: Time {timestamp}, Pred {pred_num}, Level {level}")

        # Get the data for this timestamp combination
        data_slice = ds[variable_name].sel(time=[timestamp]).isel(prediction_timedelta=[pred_num])
        ...
        if nans_before > 0:
            ...
            temp_ds = ds[[variable_name]].sel(time=[timestamp]).isel(prediction_timedelta=[pred_num])

            # Interpolate NaNs (use longitude)
            interp_temp_ds = temp_ds.interpolate_na(dim='longitude', method="linear")
            ...
            interp_temp_ds.to_zarr(zarr_path, mode='r+', region="auto") # Rewrite only region
        else:
            print("No NaNs found, skipping interpolation")

def main():
    parser = argparse.ArgumentParser(description='Interpolate NaN values in a zarr dataset at specific timestamps')
